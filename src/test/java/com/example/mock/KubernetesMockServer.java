...
public class KubernetesMockServer extends DefaultMockServer {
    ...
    private static final Context context = new io.fabric8.mockwebserver.ContextBuilder()
            .withReader(new KubernetesWebSocketReader())
            .withWriter(new DefaultWebSocketWriter())
            .build();

    public KubernetesMockServer() {
        this(false); // false = do not use HTTPS
    }
    ...
    public KubernetesMockServer(MockWebServer server, Map<io.fabric8.mockwebserver.ServerRequest, Queue<ServerResponse>> responses, boolean useHttps) {
